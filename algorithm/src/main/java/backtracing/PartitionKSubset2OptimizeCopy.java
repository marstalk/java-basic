 ...
 * 
 */
public class PartitionKSubset2OptimizeCopy {

    public static void main(String[] args) {
        PartitionKSubset2OptimizeCopy partitionKSubset = new PartitionKSubset2OptimizeCopy();
        ...
        System.out.println(partitionKSubset.canPartitionKSubsets(new int[]{2,2,2,2,3,4,5}, 4));//false
    }
    ...
    public boolean canPartitionKSubsets(int[] nums, int k) {
        ...
        int target = sum / k;
        return backtrace(k, nums, 0, 0, 0, target);
    }

    private Map<Integer, Boolean> mem = new HashMap<>();

    private boolean backtrace(int k, int[] nums, int used, int start, int bucket, int target) {
        // base
        if (k == 0) {
        ...
        if (bucket == target) {
            boolean rtn = backtrace(k - 1, nums, used, 0, 0, target);
        ...
        for (int i = start; i < nums.length; i++) {
            ...
            bucket -= nums[i];
            //TODO why &and; operation doesn't work?
            used = used & ((1 << i) - 1);
        }
